...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                        SENTINEL-D PIPELINE v3.0                         │
└─────────────────────────────────────────────────────────────────────────┘

COMPONENT 1: GHAS Webhook Input
├─ Source: GitHub Advanced Security CodeQL alerts
├─ Format: webhook_payload.json (JSON schema validated)
├─ Trigger: Real-time webhook POST to Azure Function
└─ Dev Owner: Shared (GitHub enterprise feature)

         ↓

COMPONENT 2: Azure Function Webhook Receiver
├─ Service: Azure Functions (Consumption plan)
├─ Operation: HTTP trigger → AJV schema validation → publish to Service Bus
├─ Response: 202 Accepted (async), 400 Bad Request (schema fail), 500 error
├─ Latency: 7–63ms (median 13ms local, <100ms target)
├─ Dev Owner: Dev B (Infrastructure)
└─ Next: Service Bus publish

         ↓

COMPONENT 3: Azure Service Bus Event Backbone
├─ Service: Azure Service Bus (Basic tier)
├─ Queue: vulnerability-events
├─ Features: Message lock renewal (5min default), dead-letter queue, FIFO ordering
├─ Reliability: Guaranteed delivery with 3 retries before DLQ
└─ Dev Owner: Dev B (Infrastructure)

         ↓

COMPONENT 4: SRE Agent — Telemetry Classification
├─ Language: Python 3.11+
├─ Location: /sre-agent/
├─ Function: Classify vulnerability as ACTIVE / DORMANT / DEFERRED
├─ Inputs: webhook_payload.json + raw Application Insights telemetry
├─ Process:
│  1. Generate KQL query (via Foundry with allowlist fallback)
│  2. Validate KQL query (allowlist: traces, requests, exceptions, dependencies)
│  3. Execute KQL against Application Insights
│  4. Compute "blast radius" (affected services, call frequency, user impact)
│  5. Classify: ACTIVE (telemetry hit) / DORMANT (zero hits) / DEFERRED (backlog)
├─ Output: telemetry_classification.json + routing decision
├─ Latency: 30–36ms (median 35ms local, <5 seconds target)
├─ Tests: 40/40 passing (classifier, KQL validator, router tests)
└─ Dev Owner: Dev B (Infrastructure)

         ↓ [if ACTIVE]

COMPONENT 5: Historical Database Lookup Layer
├─ Language: Python 3.11+
├─ Location: /agents/historical_db/
├─ Services: Azure Cosmos DB (serverless) + Azure AI Search
├─ Function: Find prior resolutions for this CVE or semantically similar ones
├─ Two-Stage Lookup:
│  Stage 1 (Exact): Cosmos DB partition key lookup (/cve_id) — O(1) latency
│   └─ Query: SELECT * FROM c WHERE c.cve_id = 'CVE-2021-44228'
│
│  Stage 2 (Semantic): Cosine similarity search (threshold: 0.88)
│   └─ Uses Azure OpenAI text-embedding-3-small (1536-dim vectors)
├─ Output: historical_match.json (EXACT_MATCH, SEMANTIC_MATCH, or NO_MATCH)
└─ Dev Owner: Dev A (Intelligence)

         ↓ [if NO_MATCH or enrichment]

COMPONENT 6: NLP Pipeline — Context Enrichment
├─ Language: Python 3.11+
├─ Location: /agents/nlp_pipeline/
├─ Components:
│  a) NVD Fetcher: CVSS scores, configurations, references (async HTTP)
│  b) Stack Overflow Fetcher: Top answers, code snippets (async HTTP, parallel)
│  c) spaCy NER: Entity extraction (F1 = 0.83, entities: VERSION_RANGE, API_SYMBOL, BREAKING_CHANGE, FIX_ACTION)
│  d) DistilBERT Classifier: 4-class intent (accuracy = 84.2%, classes: VERSION_PIN, API_MIGRATION, MONKEY_PATCH, FULL_REFACTOR)
├─ Output: structured_context.json (entities + intent + external knowledge)
├─ Latency: 1.8 seconds (local, <10 seconds target)
└─ Dev Owner: Dev A (Intelligence)

         ↓

COMPONENT 7: Patch Generator Agent — LLM-Powered Code Generation
├─ Language: Python 3.11+
├─ Location: /agents/patch_generator/
├─ Service: Microsoft Foundry (Azure OpenAI) + RAG Replay fallback
├─ Function: Generate safe patches or replay from history
├─ Routing:
│  • EXACT_MATCH + language compatible → RAG Replay (90 sec, no LLM call)
│  • NO_MATCH → Foundry LLM with 4-section prompt (5 min + LLM cost)
├─ 4-Section Prompt:
│  1. Context: CVE details, severity, CVSS, affected package
│  2. Intelligence: NLP entities + intent + SO solutions
│  3. Repository: Language, framework, tests, file structure
│  4. Constraints: solutions_to_avoid[], CANNOT_PATCH conditions, test requirements
├─ Output: candidate_patch.json (patch_diff, reasoning_chain, fix_strategy, confidence)
├─ Confidence Formula: (log_prob×0.40) + (constraint_adherence×0.35) + (nlp_alignment×0.25) + rag_bonus - avoidance_penalty
├─ Latency: ~90 sec (RAG) or ~5 min (Foundry)
└─ Dev Owner: Dev A (Intelligence)

         ↓

COMPONENT 8: Sandbox Validator — Test Execution + Visual Regression
├─ Language: Node.js (orchestration) + Python (SSIM analysis)
├─ Location: /sandbox-validator/
├─ Service: Azure Container Apps (ephemeral instances)
├─ Function: Validate patch in isolated environment
├─ Execution:
│  1. Container App spinup (UUID naming, <5 min)
│  2. Test suite execution (timeout: 10 min)
│  3. SSIM visual regression checking (threshold: 0.95-0.98, FPR: <5%)
│  4. Results collection + automatic teardown
├─ Output: validation_bundle.json (tests_passed, coverage_delta, visual_regression)
├─ Tests: 8/8 passing ✅ (SSIM visual regression suite)
└─ Dev Owner: Dev B (Infrastructure)

         ↓

COMPONENT 9: Safety Governor — 4-Tier Decision Router
├─ Language: Node.js (routing) + Python (decision math)
├─ Location: /safety-governor/
├─ Function: Map confidence score to graduated autonomy tier
├─ Four Tiers:
│  • HIGH (≥0.85): AUTO_PR → auto-merge eligible
│  • MEDIUM (0.70-0.85): REVIEW_PR → requires human review
│  • LOW (0.55-0.70): GITHUB_ISSUE_ESCALATE → security team review
│  • BLOCKED (<0.55): ARCHIVE → silent rejection + alert
├─ Override Conditions (can only downgrade):
│  • visual_regression === true → force MEDIUM (UI changes need eyes)
│  • touches_auth_crypto === true → force LOW (security sensitive)
│  • FULL_REFACTOR strategy → force MEDIUM (too complex for auto)
│  • CANNOT_PATCH status → force BLOCKED (no valid patch)
├─ Output: GitHub PR/Issue created with full context
├─ Latency: <3 seconds (GitHub API)
└─ Dev Owner: Dev B (Infrastructure)

         ↓

COMPONENT 10: GitHub Integration
├─ Service: GitHub API (REST v3)
├─ Function: Create PR or Issue based on Safety Governor tier
├─ PR Path (AUTO_PR or REVIEW_PR):
│  ├─ Title: "[PATCH] {CVE_ID}: {severity} in {package@version}"
│  ├─ Body: Contains CVE data, confidence score, sandbox results, reasoning chain
│  ├─ Labels: [🔒 Security], [⚠️ Code Review Required] (if MEDIUM)
│  └─ Auto-merge: Enabled for HIGH tier (CI must pass)
├─ Issue Path (GITHUB_ISSUE_ESCALATE or DORMANT):
│  ├─ Title: "[ESCALATE] {CVE_ID}: {severity} in {package@version}"
│  ├─ Template: 3-way decision gate (fix-now / defer / wont-fix labels)
│  └─ Assignee: Security team (from CODEOWNERS)
└─ Label Handlers:
   • sentinel/fix-now: Re-queue to Service Bus, override to ACTIVE
   • sentinel/defer: Write to Table Storage backlog, re-evaluate in 30 days
   • sentinel/wont-fix: Write ACCEPTED_RISK record to Cosmos DB

         ↓

COMPONENT 11: Audit Log — Append-Only Records
├─ Service: Azure Table Storage
├─ Function: Compliance-grade audit trail (insert-only, never delete/update)
├─ Fields: event_id, cve_id, severity, classification, confidence_score, tier, override_reason, timestamp
├─ Immutability: Insert-only guarantee
└─ Retention: Permanent (compliance requirement)

         ↓

COMPONENT 12: Historical Database Write — Learning Flywheel
├─ Service: Azure Cosmos DB (serverless)
├─ Trigger: After Safety Governor decision
├─ Schema: historical_db_record.json
├─ Fields:
│  ├─ cve_id (partition key, enables O(1) lookups)
│  ├─ patch_outcome (SUCCESS / PARTIAL / FAILED / ACCEPTED_RISK)
│  ├─ patch_diff (full unified diff)
│  ├─ fix_strategy_used (VERSION_PIN / API_MIGRATION / MONKEY_PATCH / FULL_REFACTOR)
│  ├─ solutions_tried (array of prior attempts: [{strategy, outcome}])
│  ├─ confidence_score (final composite score from this resolution)
│  ├─ embedding_vector (1536-dim for semantic search)
│  └─ resolved_at (ISO 8601 timestamp)
├─ Operation: Upsert (INSERT or UPDATE)
├─ Latency: <500ms target
└─ Result: EXACT_MATCH availability for next similar CVE
